...
module.exports = class extends Command {
    constructor(...args) {
      super(...args, {
        name: 'help',
        aliases: ["help", "menu"],
        description: `Display\'s ${config.bot_name || 'Bot'}\'s Hilfe Menü.`,
    ...
    .setColor('GREEN')
    .setTitle(`${config.bot_name || 'Bot'}'Command Liste`)
    ...
    message.channel.send(embed)
        ...
        if(!cmd) return message.channel.send(`Kann nicht den Command finden.`);

        const embed = new MessageEmbed()
        embed.setTitle(`Command: ${cmd.name}`)
        embed.setDescription(cmd.description)
        embed.setFooter(cmd.disabled ? 'Dies ist gerade deaktiviert.' : message.member.displayName, message.author.displayAvatarURL({ dynamic: true }))
